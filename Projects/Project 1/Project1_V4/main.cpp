 ...
 * Created on January 26, 2024, 11:26 PM
 * Purpose: HangMan
 */
...
int main(int argc, char** argv) {
    ...
    if(chWrd == 1){
        do{
            cout<<"Guess a letter:";
            cin>>usrLtr;
            if (usrLtr == 't' || usrLtr == 'T' || usrLtr == 'r' || usrLtr == 'R' || usrLtr == 'E' || usrLtr == 'e'){
                cout<<usrLtr<<endl;   
            }
            for (int i=0; i<gsWrd.length(); i++){
                if(gsWrd.length() == usrLtr){
                    cout<<usrLtr;
                }
            }
        ...
        }while(usrLtr != 't' && usrLtr != 'T' && usrLtr != 'r' && usrLtr != 'R' && usrLtr != 'E' && usrLtr != 'e');
    }
    if(chWrd == 2){
    ...
    //Lose condition, End the Game
    if (lives == 0) {
        cout<< "Game over! You ran out of lives. The word was: "<< gsWrd <<endl;
    }  
    //Win condition, User guesses correctly
    //Close file
    wrdBnk.close();
    //Exit the program
    return 0;
